...
function MapPreview() {
    ...
    return (
        <Link 
            to={`/map/${activeBook?.id}`} 
            style={{
                display: 'flex', 
                width: '100%', height: '100%', 
                textDecoration: 'none', 
                overflow: 'hidden',
                pointerEvents: activeBook?.id == undefined ? "none" : "auto"
            }}
        >
...
function MapInfo() {
    ...
    <>
        ...
        Принять участие в данном проекте может любой желающий, по этому вопросу можно связаться с автором.</Text>
    </>
    )
}

...
use shared_gen::book_service::{book_service_server, CommonResponse, GetAllWizformsRequest, GetCompleteWizformRequest, GetCompleteWizformResponse, GetWizformsRequest, GetWizformsResponse, InsertItemsBulkRequest};
use tonic::{Request, Response, Status};
use uuid::Uuid;
use crate::services::book::models::wizform;
...
#[tonic::async_trait]
impl book_service_server::BookService for BookServiceImpl {
    async fn get_all_wizforms(&self, request: Request<GetAllWizformsRequest>) -> Result<Response<GetWizformsResponse>, Status> {
        let book_id = Uuid::from_str(&request.get_ref().book_id).map_err(|err| Status::invalid_argument(err.to_string()))?;
        let wizforms = wizform::Entity::find()
        ...
        Ok(Response::new(GetWizformsResponse { wizforms }))
    }
    ...
    async fn get_complete_wizform(&self, request: Request<GetCompleteWizformRequest>) -> Result<Response<GetCompleteWizformResponse>, Status> {
        todo!()
    }

    async fn insert_items_bulk(&self, request: Request<InsertItemsBulkRequest>) -> Result<Response<CommonResponse>, Status> {
        todo!()
    }
}

...
function SectionsGrid({sections, bookId}: {
    ...
    <>
        {
        sections == undefined ? <LoadingOverlay/> : 
            <SimpleGrid cols={{sm: 1, lg: 2}} className={classes.map_grid}>{sections?.map((s, i) => (
                <Link key={i} to={`locations/${s.id}`} style={{textDecoration: 'none'}}>
                    <Card 
                        withBorder 
                        radius={0} 
                        style={{

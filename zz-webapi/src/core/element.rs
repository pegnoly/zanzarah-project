use axum::{extract::{Path, Query, State}, routing::{get, patch}, Json, Router};
use uuid::Uuid;
use zz_data::core::wizform::ElementDBModel;

use super::{queries::ElementUpdateQuery, utils::{ApiManager, StringPayload}};
...
async fn get_existing_elements(
    ...
    let res: Result<Vec<ElementDBModel>, sqlx::Error> = sqlx::query_as(r#"
    ...
    match res {
        ...
        Err(query_failure) => {
            tracing::info!("Error fetching elements of book {}: {}", &book_id, query_failure.to_string());
            Err(())
        }
    }
}
...
async fn update_element(
    ...
    let res: Result<ElementDBModel, sqlx::Error> = sqlx::query_as(r#"
            UPDATE elements 
    ...
    match res {
        Ok(r) => {
            tracing::info!("Element {:?} updated successfully", &update_element_query.name);
            Ok(())
        },
        Err(e) => {
            tracing::info!("Failed updating element {}: {}", &update_element_query.name, e.to_string());
            Err(())
        }
    }
}